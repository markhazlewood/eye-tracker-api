...
/**
 * A class used to set up communication with the mTrackerServerCommandAddress eye tracking device.
 * This class runs on a seperate thread. Necessary, because otherwise issues
 * occur when communicating with the mTrackerServerCommandAddress and trying to draw to the GLCanvas
 * at the same time.
 * 
...
{  	
   ...
   private final String OUTPUT_PATH = "C:" + File.separator +  "dataDump.txt"; //"C:" + File.separator + "Users" + File.separator + "cde7825" + File.separator + "Desktop" + File.separator + "dataDump.txt"
   ...
   private final String DATA_REQUEST_COMMAND = "ET_STR " + Integer.toString(mDesiredDataRate) + "\n";       // TODO: This should be a member, not a constant, in case mDesiredDataRate changes at runtime
   ...
   // <editor-fold defaultstate="expanded" desc="Constructor(s)">
	 ...
	 * 
	 * @param filter
	 *            - the cursor associated with this IViewXComm
	 */
	public IViewXComm(GazePoint cursor)
   {
		super(cursor);
	...
	/**
	 * Constructor for the IViewXComm class. Sets the cursor object associated
	 * with this IViewXComm instance to cursor and the ip of the system running mTrackerServerCommandAddress
	 * 
	 * @param filter
	 *            - the cursor associated with this IViewXComm
	 * @param ipAddress
   ...
   // <editor-fold defaultstate="expanded" desc="Working Functions">
	...
	/**
	 * Overrides the run method. First sets up UDP connection with the mTrackerServerCommandAddress,
	 * then enters a loop and continuously tries to receive data from the mTrackerServerCommandAddress
